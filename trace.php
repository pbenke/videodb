...
function _replace_enclosed_tag_traced($matches): string
{
    ...
    if ($engine == 'imdb')
    {
        ...
        if (preg_match("#=(.*?)\&ref_=ttep_(ep|yr)_sn_(pv|nx)#", $matches[2], $mymatches)) {
//          echo "<BR> in matches"; var_dump($matches);
//          echo "mymatches 1"; var_dump($mymatches);
//          echo '<BR> $url - '.$url;
            if (!preg_match('#(/episodes/\?(season|year)=)#', $url, $mymatches)) {
//                echo "<BR> mymatches 2"; var_dump($mymatches);
                $patterns = ['#(\?season)#', '#(\?year)#'];
                $replacements = ['episodes?season','episodes?year'];
